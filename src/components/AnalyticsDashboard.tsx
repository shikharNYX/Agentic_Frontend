        ...
        <Card className="p-6 bg-[#1A0B2E]/80 border-[#6D28D9]/20 backdrop-blur-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-2 mb-6">
            {accountMetrics.map((metric) => (
              <Card key={metric.label} className="bg-[#2A1A4D]/90 border-[#6D28D9]/30 text-left p-1 w-full">
                <CardHeader>
                  <CardTitle className="text-xs font-semibold text-white">{metric.label}</CardTitle>
                  <CardDescription className="text-purple-200 text-xs">{metric.value}</CardDescription>
                </CardHeader>
                <CardContent>
                  <p className={`text-xs ${metric.change > 0 ? 'text-green-400' : 'text-red-400'}`}>Change: {metric.change}%</p>
                </CardContent>
              </Card>
            ))}
          </div>
